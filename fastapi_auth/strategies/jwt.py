...
class JwtStrategy(Strategy[user_model, None]):
    ...
    async def read_token(self, token: Optional[str]) -> Optional[user_model]:
        """
        Read jwt token and returns the user
        ...
        try:
            ...
            if datetime.now().timestamp() > lifetime:
                return None
    ...
    async def get_token_by_user(self, user: user_model) -> tuple[str, str]:
        ...
        return (encode_jwt(refresh_data, lifetime_seconds=self.refresh_token_lifetime_seconds),
                encode_jwt(access_data, lifetime_seconds=self.access_token_lifetime_seconds))
